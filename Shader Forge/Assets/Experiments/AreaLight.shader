...
/////// Normals:
                ...
                float3 normalLocal = UnpackNormal(tex2D(_Normals,TRANSFORM_TEX(node_162.rg, _Normals))).rgb;
...
////// Lighting:
                ...
                float3 finalColor = (((_DiffuseContribution*attenuation*max(0,dot(normalize((node_20-lerp((node_59_if_leA*node_55)+(node_59_if_leB*(normalize(node_55)*node_24)),node_55,node_59_if_leA*node_59_if_leB))),normalDirection))*tex2D(_Diffuse,TRANSFORM_TEX(node_162.rg, _Diffuse)).rgb)+(pow(max(0,dot(normalize((viewDirection+normalize((node_20-lerp((node_27_if_leA*node_17)+(node_27_if_leB*(normalize(node_17)*node_24)),node_17,node_27_if_leA*node_27_if_leB))))),normalDirection)),exp2((((_Gloss*tex2D(_Glossmap,TRANSFORM_TEX(node_162.rg, _Glossmap)).rgb)*10.0)+1.0)))*_Spec))*_LightColor0.rgb);
/// Final Color:
...
/////// Normals:
                ...
                float3 normalLocal = UnpackNormal(tex2D(_Normals,TRANSFORM_TEX(node_163.rg, _Normals))).rgb;
                float3 normalDirection =  mul( normalLocal, tangentTransform ); // Perturbed normals
                float3 viewReflectDirection = reflect( -viewDirection, normalDirection );
////// Lighting:
                float attenuation = LIGHT_ATTENUATION(i);
                ...
                float node_27_if_leA = step(length(node_17),node_24);
                float node_27_if_leB = step(node_24,length(node_17));
                float3 finalColor = (((_DiffuseContribution*attenuation*max(0,dot(normalize((node_20-lerp((node_59_if_leA*node_55)+(node_59_if_leB*(normalize(node_55)*node_24)),node_55,node_59_if_leA*node_59_if_leB))),normalDirection))*tex2D(_Diffuse,TRANSFORM_TEX(node_163.rg, _Diffuse)).rgb)+(pow(max(0,dot(normalize((viewDirection+normalize((node_20-lerp((node_27_if_leA*node_17)+(node_27_if_leB*(normalize(node_17)*node_24)),node_17,node_27_if_leA*node_27_if_leB))))),normalDirection)),exp2((((_Gloss*tex2D(_Glossmap,TRANSFORM_TEX(node_163.rg, _Glossmap)).rgb)*10.0)+1.0)))*_Spec))*_LightColor0.rgb);
/// Final Color:
