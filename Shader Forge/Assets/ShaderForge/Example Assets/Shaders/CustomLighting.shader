    ...
    SubShader {
        ...
        Pass {
            ...
            fixed4 frag(VertexOutput i) : COLOR {
                ...
                float3 normalLocal = UnpackNormal(tex2D(_Normals,TRANSFORM_TEX(node_270.rg, _Normals))).rgb;
...
////// Lighting:
                ...
                float3 finalColor = (attenuation*_LightColor0.rgb*((tex2D(_Diffuse,TRANSFORM_TEX(node_270.rg, _Diffuse)).rgb*_Color.rgb*(round((max(0,dot(lightDirection,node_41))*node_216))/node_216))+UNITY_LIGHTMODEL_AMBIENT.rgb+(round((node_216*pow(max(0,dot(node_41,halfDirection)),exp2(((_Gloss*10.0)+1.0)))))/node_216)));
/// Final Color:
