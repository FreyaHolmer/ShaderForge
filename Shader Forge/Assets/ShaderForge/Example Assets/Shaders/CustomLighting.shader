    ...
    SubShader {
        ...
        Pass {
            ...
            fixed4 frag(VertexOutput i) : COLOR {
                ...
                float3 normalLocal = UnpackNormal(tex2D(_Normals,TRANSFORM_TEX(i.uv0.xy, _Normals))).rgb;
...
////// Lighting:
                ...
                float3 lightFinal = ((attenuation*(((tex2D(_Diffuse,TRANSFORM_TEX(i.uv0.xy, _Diffuse)).rgb*_Color.rgb)*max(0,dot(lightDirection,node_41)))+pow(max(0,dot(node_41,halfDirection)),50.0)))*_LightColor0.rgb)+UNITY_LIGHTMODEL_AMBIENT.xyz;
/// Final Color:
...
////// Lighting:
                ...
                float3 attenColor = attenuation * _LightColor0.xyz;
                float3 node_41 = normalDirection;
                float3 lightFinal = ((attenuation*(((tex2D(_Diffuse,TRANSFORM_TEX(i.uv0.xy, _Diffuse)).rgb*_Color.rgb)*max(0,dot(lightDirection,node_41)))+pow(max(0,dot(node_41,halfDirection)),50.0)))*_LightColor0.rgb);
/// Final Color:
                return fixed4(lightFinal,1);
            }
            ENDCG
