...
////// Emissive:
                ...
                float3 emissive = ((_ConeStrength*_ConeColor.rgb)*(tex2D(_ConeFalloff,TRANSFORM_TEX(float2(pow(1.0-max(0,dot(normalDirection, viewDirection)),0.5),i.uv0.rg.g), _ConeFalloff)).r*lerp(0.4,1,tex2D(_Smoke,TRANSFORM_TEX(((float2(node_244.r,node_244.b)+node_297.r*float2(1,1))*_SmokeScale), _Smoke)).r)));
