...
/////// Normals:
                ...
                float3 normalLocal = lerp(UnpackNormal(tex2D(_FloorNormal,TRANSFORM_TEX(node_590.rg, _FloorNormal))).rgb,normalize(lerp(UnpackNormal(tex2D(_EdgeNormal,TRANSFORM_TEX(node_400, _EdgeNormal))).rgb,UnpackNormal(tex2D(_EdgeNormal,TRANSFORM_TEX(node_438, _EdgeNormal))).rgb,0.5)),node_132.b);
...
/////// Diffuse:
                ...
                #endif
///////// Gloss:
                float4 node_2 = tex2D(_MainTex,TRANSFORM_TEX(node_590.rg, _MainTex));
...
/////// Normals:
                ...
                float2 node_438 = (node_399.rg*node_407*0.5);
                float4 node_132 = i.vertexColor;
                float3 normalLocal = lerp(UnpackNormal(tex2D(_FloorNormal,TRANSFORM_TEX(node_591.rg, _FloorNormal))).rgb,normalize(lerp(UnpackNormal(tex2D(_EdgeNormal,TRANSFORM_TEX(node_400, _EdgeNormal))).rgb,UnpackNormal(tex2D(_EdgeNormal,TRANSFORM_TEX(node_438, _EdgeNormal))).rgb,0.5)),node_132.b);
                float3 normalDirection =  mul( normalLocal, tangentTransform ); // Perturbed normals
...
/////// Diffuse:
                ...
                float3 diffuse = max( 0.0, NdotL) * attenColor;
///////// Gloss:
                float4 node_2 = tex2D(_MainTex,TRANSFORM_TEX(node_591.rg, _MainTex));
                float gloss = exp2(lerp((0.4*node_2.a),0.8,node_132.b)*10.0+1.0);
////// Specular:
                NdotL = max(0.0, NdotL);
                float node_146 = lerp(0.1,0.8,node_132.b);
