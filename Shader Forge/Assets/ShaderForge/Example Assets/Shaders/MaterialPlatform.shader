...
Shader "Shader Forge/MaterialPlatform" {
    Properties {
    ...
    SubShader {
        ...
        Pass {
            ...
            fixed4 frag(VertexOutput i) : COLOR {
                ...
                float3 normalLocal = lerp(UnpackNormal(tex2D(_FloorNormal,TRANSFORM_TEX(node_587.rg, _FloorNormal))).rgb,normalize(lerp(UnpackNormal(tex2D(_EdgeNormal,TRANSFORM_TEX((node_399.rg*node_407), _EdgeNormal))).rgb,UnpackNormal(tex2D(_EdgeNormal,TRANSFORM_TEX((node_399.rg*node_407*0.5), _EdgeNormal))).rgb,0.5)),node_132.b);
...
///////// Gloss:
                float4 node_2 = tex2D(_Floor,TRANSFORM_TEX(node_587.rg, _Floor));
...
/// Final Color:
        ...
        Pass {
            ...
            fixed4 frag(VertexOutput i) : COLOR {
                ...
                float node_407 = _EdgeNormalScale;
                float4 node_132 = i.vertexColor;
                float3 normalLocal = lerp(UnpackNormal(tex2D(_FloorNormal,TRANSFORM_TEX(node_588.rg, _FloorNormal))).rgb,normalize(lerp(UnpackNormal(tex2D(_EdgeNormal,TRANSFORM_TEX((node_399.rg*node_407), _EdgeNormal))).rgb,UnpackNormal(tex2D(_EdgeNormal,TRANSFORM_TEX((node_399.rg*node_407*0.5), _EdgeNormal))).rgb,0.5)),node_132.b);
                float3 normalDirection = normalize( mul( normalLocal, tangentTransform ) );
...
/////// Diffuse:
                ...
                float3 diffuse = max( 0.0, NdotL) * attenColor;
///////// Gloss:
                float4 node_2 = tex2D(_Floor,TRANSFORM_TEX(node_588.rg, _Floor));
                float gloss = exp2(lerp((0.4*node_2.a),0.8,node_132.b)*10.0+1.0);
////// Specular:
                NdotL = max(0.0, NdotL);
                float node_146 = lerp(0.1,0.8,node_132.b);
