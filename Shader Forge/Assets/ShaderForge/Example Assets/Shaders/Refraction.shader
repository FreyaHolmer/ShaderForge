...
////// Specular:
                ...
                float3 finalColor = diffuse * lerp(_TileA.rgb,_TileB.rgb,abs((1.0 - (node_66.r+node_66.g)))) + specular;
/// Final Color:
                return fixed4(lerp(tex2D(_GrabTexture, float2(1,grabSign)*i.screenPos.xy*0.5+0.5 + (node_25.rgb.rg*_RefractionIntensity)).rgb, finalColor,0.3),1);
            }
            ENDCG
        }
        Pass {
