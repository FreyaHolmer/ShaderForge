        ...
        Pass {
            ...
            VertexOutput vert (VertexInput v) {
                ...
                void displacement (inout VertexInput v){
                    ...
                    float node_15 = 1.0;
                    float4 node_152 = tex2Dlod(_DisplacementR,float4(TRANSFORM_TEX((node_6+*float2(1,0)), _DisplacementR),0.0,node_15));
...
////// Specular:
                NdotL = max(0.0, NdotL);
                float3 node_162 = lerp(float3(0.4117647,0.3826572,0.3602941),(tex2D(_Diffuse,TRANSFORM_TEX(i.uv0.xy, _Diffuse)).rgb*tex2D(_AORGBHeightA,TRANSFORM_TEX(node_6, _AORGBHeightA)).rgb),node_156);
                float node_165 = (node_156*node_162.r);
                float3 specular = attenColor * float3(node_165,node_165,node_165) * pow(max(0,dot(reflect(-lightDirection, normalDirection),viewDirection)),gloss);
                float3 lightFinal = diffuse * node_162 + specular;
/// Final Color:
