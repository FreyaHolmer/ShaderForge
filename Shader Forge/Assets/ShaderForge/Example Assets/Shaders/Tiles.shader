    ...
    SubShader {
        ...
        Pass {
            ...
            fixed4 frag(VertexOutput i) : COLOR {
                ...
                float3 normalLocal = UnpackNormal(tex2D(_Normal,TRANSFORM_TEX(node_162.rg, _Normal))).rgb;
...
///////// Gloss:
                float4 node_138 = tex2D(_Diffuse,TRANSFORM_TEX(node_162.rg, _Diffuse));
...
////// Specular:
                ...
                float3 finalColor = diffuse * node_138.rgb + specular;
/// Final Color:
        ...
        Pass {
            ...
            fixed4 frag(VertexOutput i) : COLOR {
                ...
                float3 normalLocal = UnpackNormal(tex2D(_Normal,TRANSFORM_TEX(node_163.rg, _Normal))).rgb;
                float3 normalDirection = normalize( mul( normalLocal, tangentTransform ) );
...
///////// Gloss:
                float4 node_138 = tex2D(_Diffuse,TRANSFORM_TEX(node_163.rg, _Diffuse));
                float gloss = exp2((pow(node_138.r,2.0)*_Gloss)*10.0+1.0);
////// Specular:
                NdotL = max(0.0, NdotL);
                float _Specular_var = _Specular;
